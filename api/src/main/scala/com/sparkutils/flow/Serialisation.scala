...
@SerialVersionUID(1L)
case class StepRow(flowId: Int, flowVersion: Int, index: Int, ruleSuiteId: Int, ruleSuiteVersion: Int, inputView: String,
...
trait Serialisation {
  ...
  def loadSteps(dataFrame: DataFrame, flowId: Column, flowVersion: Column,
                ruleSuiteId: Column, ruleSuiteVersion: Column, index: Column, inputView: Column, operation: Column,
    ...
    dataFrame.select(
      flowId.as("flowId"), flowVersion.as("flowVersion"),
    ...
    ).orderBy(index).as[StepRow]
  }

  def loadSteps(dataFrame: DataFrame): Dataset[StepRow] =
    loadSteps(dataFrame, col("flowId"), col("flowVersion"),
      col("ruleSuiteId"), col("ruleSuiteVersion"), col("index"), col("inputView"), col("operation"),
  ...
  def toDatasets(sparkSession: SparkSession, flow: Flow): FlowDataSets = {
    ...
    FlowDataSets(
      flow.steps.zipWithIndex.map{case (step, index) =>
        StepRow(flowId = flow.flowId.id, flowVersion = flow.flowId.version, index = index,
  ...
  def fromDatasets(sparkSession: SparkSession, flowDataSets: FlowDataSets, flowId: VersionedId): (Seq[Step], String) = {
    ...
    val flowSteps = steps.filter(flowFilter).orderBy("index")
    ...
    val thisSteps =
      flowSteps.collect().map{
        step =>
          ...
          Step(rs, step.inputView, views, step.operation,
